...
### Get ARM Provider Locations
{
    ...
    (Get-AzureRmResourceProvider   -ProviderNamespace "$resourceProviderNamespace"| `
            Where-Object {$_.ResourceTypes.ResourceTypeName -eq "$resourceTypeName"}).Locations |?{$_ -like '*europ*'} 
}
