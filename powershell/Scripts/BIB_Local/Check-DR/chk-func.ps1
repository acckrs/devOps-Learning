...
function global:Chk-Ping {
    $pingOk = $true;
    $hostName = $oName.trim();
    if ($oName -match '([a-z0-9]+)\$.+') {
        $hostName = $Matches[1]
    }
    $oPing = new-object System.Net.NetworkInformation.Ping
    try {
        $res = $oPing.send($hostname)    
    }
    catch  [System.Net.NetworkInformation.PingException] {
        $mm = "Host name " + $hostName + " cannot be resolved to IP address "
        log -msg $mm
        $PingOK = $false
    }
    $mm = "Ping reply sa adrese " + $res.address.IPAddressToString + " je ", $res.status
    if ($pingOK) {
