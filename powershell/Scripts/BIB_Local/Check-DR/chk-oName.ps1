...
#prihvat argumenta
#$global:logFile=Get-Item 'c:\acca\powershell_local\BIB\Check-DR\toCheck\OS.VM~BERZA2.log';
if ($args.Length -eq 0) {Throw ('Nije naveden log fajl!');}
#$global:logFile=Get-Item 'toCheck\OS.VM~BERZA2.log';
...
try{
  ...
  $IsWindows=Get-Content $logFile | ?{$_ -match 'osName[ ]+:(.+)'} | %{$_ -like '*Windows*'};
