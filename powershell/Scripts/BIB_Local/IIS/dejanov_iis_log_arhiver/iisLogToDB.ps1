...
$files =  dir $path -Filter *.log | sort CreationTime -Descending | select -Skip 1
...
{
    $logs = ConvertFrom-IISLog $file.fullname |  select @{l='hostname';e={$hostname}},@{l='appPool';e={$appPool}},Datetime,IP,Method,uri,port,username,agent,status,substatus,win32status,@{l='TimeTaken';e={if (!$_.timetaken) { [System.DBNull]::Value } else {[int] $_.timeTaken} }},BytesSent,BytesReceived
    ...
    {        
        ...
        if ($PSBoundParameters.ContainsKey("LogujSve"))
        {
