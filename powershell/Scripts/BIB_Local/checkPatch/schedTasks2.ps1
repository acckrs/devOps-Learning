function disable-scheduledTask {
    ...
    PROCESS {
        ...
        foreach ($computer in $computerName) {
            write-verbose "Disabling task $t on $computer"
            $results = @()
            try {
                $OK = $true
                $p = start-process schtasks -argumentList  "/change /s $computer  /tn wsus /disable" -wait -PassThru -ErrorAction SilentlyContinue
                if ($p.ExitCode -ne 0) {
                    write-error "Failed with error $($p.exitcode)"
                }
            }
            catch {
                $OK = $false
